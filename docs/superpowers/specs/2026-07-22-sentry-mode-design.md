...
| Tilt during sweep | **Held** at its current angle — a lost target's tilt is the best prior for where it will reappear. |
...
never reports its pose. The host therefore keeps a **pan estimate**:

- Starts at **90.0°** — the Pi's `ServoDriver` recenters to `CENTER` (90/90) on startup.
- Every `pan_delta` the sink ships (tracking **and** sentry alike) is accumulated
  into the estimate, then clamped to 0–180 with the same limits the Pi applies
  (`turret_pi/servo.py`: `PAN_MIN_DEG = 0.0`, `PAN_MAX_DEG = 180.0`).
- **Self-correction at the limits:** if UDP loss makes the estimate drift from the
...
def observe_aim(state: SentryState, pan_delta: float) -> SentryState:
    """A locked frame shipped `pan_delta`: fold it into the estimate (clamped),
    reset the grace timer and the sweep direction (next sweep starts left)."""

def step(state: SentryState, dt: float, config: SentryConfig) -> tuple[float, SentryState]:
    ...
    max_delta_deg), advance the estimate, and flip direction at 0/180."""
```
...
3. `pan_estimate_deg` reflects **every** shipped pan delta, tracking or sentry.
4. Estimate is always clamped to [0, 180]; hitting a bound flips `direction`.
5. A single step's output never exceeds `max_delta_deg` (stall protection: a huge
   `dt` from a pipeline hiccup must not produce a violent jump).
6. `dt = 0` (first frame) emits no motion.
7. Sweep frames emit `tilt_delta = 0` — tilt is held.
...
- **Locked:** unchanged aim path; additionally `observe_aim(state, command.pan_delta)`
  so the estimate tracks reality.
- **Unlocked:** `step(state, dt, config)`; when it yields a nonzero delta, ship
  `AimCommand(pan_delta=delta, tilt_delta=0.0)` through the same transport/seq path.
...
SENTRY_SPEED_DEG_S = 15.0     # continuous sweep speed
...
tracks --> ActuatorSink.show
             ...
             └─ unlocked --> sentry.step(state, dt) ─ delta? ───┴─> send AimCommand(delta, 0)
...
- **UDP loss:** estimate drifts mid-sweep, bounded, re-zeroed at each limit (see
  dead-reckoning section). Accepted.
- **Sender restart:** host restarts → estimate resets to 90 but the Pi may not be at
  90. Worst case the first sweep is asymmetric; the first limit hit re-syncs it.
  Accepted for v1 (same spirit as ADR-0013's "freshest command wins").
...
- direction flips at 0 and at 180; estimate never leaves [0, 180]
- `observe_aim` accumulates tracking deltas and clamps
- large `dt` → output clamped to `max_delta_deg`
- `dt = 0` → zero output
- sweep output always has `tilt_delta` contribution of exactly 0

Extended `tests/test_actuator_sink.py`:

- unlocked frames within grace → transport receives nothing (current behaviour holds)
- unlocked past grace → transport receives sweep commands, `tilt_delta == 0`
- target appears mid-sweep → very next frame ships an aim command, not a sweep one
- estimate consistency: aim deltas shipped while locked shift where the sweep resumes
